...
%
global qf qdf sigma tau m J obs obsOrder obsScale;
T = 1;     %10
...
%
TrajList = traj.trajList(x, xd, xdd, xddd, xdddd, y, yd, ydd, yddd, ydddd, th, thd, thdd, thddd, thdddd, mu, mud);

%-- Parameter list (What role does this play in overall setup????)
%
ParamList = { 'qf', 'qdf', 'sigma', 'm', 'J', 'tau', 'obs', 'obsOrder', 'obsScale'};
...
dynamics ={'-thdddd+sigma*thdd-mu/J*(xd*cos(th)+yd*sin(th))',... 
            '-xdddd+sigma*xdd+tau*obsOrder/2*((x-obs(1))/obsScale(1))^(obsOrder-1)/obsScale(1)/(((x-obs(1))/obsScale(1))^obsOrder+((y-obs(2))/obsScale(2))^obsOrder-1^obsOrder)^2+1/m*(mud*sin(th)+mu*thd*cos(th))',...
            '-ydddd+sigma*ydd+tau*obsOrder/2*((y-obs(2))/obsScale(2))^(obsOrder-1)/obsScale(2)/(((x-obs(1))/obsScale(1))^obsOrder+((y-obs(2))/obsScale(2))^obsOrder-1^obsOrder)^2+1/m*(-mud*cos(th)+mu*thd*sin(th))'};
...
init = [xsp.coefs ysp.coefs thsp.coefs musp.coefs];
...
init_forward = [THcol(1),THdcol(1),THddcol(1),THdddcol(1),...
                ...
                mu0Opt];
tspan = linspace(0, tfinal,scale);
[tt_forward,xx_forward] = ode15s(@ForwardVarPathPlan,[0 tfinal],init_forward');
...
fh1= figure(2);
  subplot(2,3,1);
   ...
   axis square;
    ...
    subplot(2,3,3);
      dynamics_TH_opt = -THddddcol+sigma.*THddcol-1/J.*MUcol.*(Xdcol.*cos(THcol)+Ydcol.*sin(THcol));
   ...
   axis square;
    ...
    subplot(2,3,4);
      dynamics_X_opt = -Xddddcol+sigma.*Xddcol+tau.*obsOrder/2.*((Xcol-obs(1))./obsScale(1)).^(obsOrder-1)./obsScale(1)./(((Xcol-obs(1))./obsScale(1)).^obsOrder+((Ycol-obs(2))./obsScale(2)).^obsOrder-1^obsOrder).^2+1/m.*(MUdcol.*sin(THcol)+MUcol.*THdcol.*cos(THcol));
    
    plot(HL,dynamics_X_opt,'b-o');xlabel('time'); ylabel('Error(X dynamics)');
   axis square;
    xlim([0, tfinal])
      subplot(2,3,5);
      dynamics_Y_opt = -Yddddcol+sigma.*Yddcol+tau.*obsOrder/2.*((Ycol-obs(2))./obsScale(2)).^(obsOrder-1)./obsScale(2)./(((Xcol-obs(1))./obsScale(1)).^obsOrder+((Ycol-obs(2))./obsScale(2)).^obsOrder-1^obsOrder).^2+1/m.*(-MUdcol.*cos(THcol)+MUcol.*THdcol.*sin(THcol));
